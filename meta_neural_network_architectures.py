...
from psoftplus import ParametricSoftplus, get_activation_obj, ApplyOverHalves, ChannelRepeatOnce, SumHalves, RowView
...
class MetaMaxResLayerReLU(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, stride, padding, use_bias, args, normalization=True,
                 ...
                 use_curvature=False, clip_bn=False, init_lipschitz=10.0):
        """
        ...
        ## curvature analysis support ##
        self.use_curvature = use_curvature
        self.clip_bn = clip_bn
        self.init_lipschitz = init_lipschitz
        if self.use_curvature:
            self.activation_obj = get_activation_obj(activation_name='parametric_softplus')
        else:
            self.activation_obj = get_activation_obj(activation_name='softplus')
        self.build_block()
    ...
    def build_block(self):
        ...
        # self.layer_dict['activation_function_pre'] = self.activation_obj()
        # out = self.activation_obj().forward(self.conv.forward(out))
        self.layer_dict['activation_function_pre'] = self.activation_obj()
        ...
        if self.normalization:
            if self.args.norm_layer == "batch_norm":
                self.norm_layer = MetaBatchNormLayer(out.shape[1], track_running_stats=True,
                                                     ...
                                                     use_per_step_bn_statistics=self.use_per_step_bn_statistics,
                                                     args=self.args,
                                                     use_curvature=self.clip_bn, init_lipschitz=self.init_lipschitz)
...
# add spectual normalization for curvature analysis #
...
def convspectralnorm_wrapper(module: torch.nn.Module,
                             ...
                             n_power_iterations: int=1):
    with P.cached():
        out = P.register_parametrization(module, "weight", 
                                ConvSpectralNorm(weight=module.weight, 
                                                 stride=module.stride,
                                                 padding=module.padding,
                                                 im_size=im_size,
                                                 n_power_iterations=n_power_iterations))
        # print('===Conv Spectral Norm===')
        # print(out)
    return out

class MetaConv2dLayer(nn.Module):
    def __init__(self, 
                 in_channels, out_channels, kernel_size, stride, padding, use_bias, groups=1, dilation_rate=1, 
                 use_curvature=False,):
        """
        ...
        ## TODO: add conv layer spectral norm for curvature analysis ##
        # if use_curvature:
        #     self = convspectralnorm_wrapper(self)
    ...
    def forward(self, x, params=None):
        ...
        if params is not None:
            ...
            params = extract_top_level_dict(current_dict=params)
            # print(params.keys())
            # print(params["parametrizations"].keys())
            if "weight" not in params.keys():
                params["weight"] = params["parametrizations"]['weight.original']
            if self.use_bias:
...
# add spectral norm on linear weights for curvature analysis #
class MetaLinearLayer(nn.Module):
    def __init__(self, input_shape, num_filters, use_bias,
        ...
        # if use_curvature:
        #     torch.nn.utils.parametrizations.spectral_norm(self, name='weights')
    ...
    def forward(self, x, params=None):
        ...
        """
        if self.use_curvature:
            self = torch.nn.utils.parametrizations.spectral_norm(self, name='weights')
        if params is not None:
            # print('=========== Params (Linear) ============')
            # print(params.keys())
            params = extract_top_level_dict(current_dict=params)
            # print('=========== Params (Linear-after extract) ============')
            # print(params['parametrizations'])
            if "weights" not in params.keys():
                params["weights"] = params["parametrizations"]['weights.original']
            if self.use_bias:
...
## add lipschitz constraint for curvature analysis ##
class MetaBatchNormLayer(nn.Module):
    def __init__(self, num_features, device, args, eps=1e-5, momentum=0.1, affine=True,
                 ...
                 use_curvature=False, init_lipschitz: float = math.inf, ):
        """
    ...
    def forward(self, input, num_step=1, params=None, training=False, backup_running_statistics=False):
        ...
        ## curvature analysis support ##
        if self.use_curvature:
            scale = torch.min((self.running_var + self.eps) ** .5)
            one_lipschitz_part = output * scale
            output = one_lipschitz_part * torch.minimum(1 / scale, self.log_lipschitz.exp())
        ##############################
...
class VGGReLUNormNetwork(nn.Module):
    ...
    def build_network(self):
        ...
        self.layer_dict['linear'] = MetaLinearLayer(input_shape=(out.shape[0], np.prod(out.shape[1:])),
                                                    ...
                                                    use_curvature=self.use_curvature,)
        # if self.use_curvature:
        #     self.layer_dict['linear'] = torch.nn.utils.parametrizations.spectral_norm(self.layer_dict['linear'], name='weights')
