...
from psoftplus import ParametricSoftplus, get_activation_obj
    ...
    def __init__(self, im_shape, device, args):
        ...
        if self.args.backbone == 'ResNet12':
            self.classifier = ResNet12(im_shape=self.im_shape, num_output_classes=self.args.
                                                 num_classes_per_set,
                                                 args=args, device=device, meta_classifier=True, 
                                                 use_curvature=args.use_curvature_inner, clip_bn=args.clip_bn, init_lipschitz=args.init_lipschitz).to(device=self.device)
        else:
        ...
        if self.args.alfa:
            ...
            if self.args.use_curvature_outer:
                ...
                self.regularizer_activation_obj = get_activation_obj(activation_name='softplus')
            
            self.regularizer = nn.Sequential(
                fc1,
                self.regularizer_activation_obj(),
                fc2
            ).to(device=self.device)
